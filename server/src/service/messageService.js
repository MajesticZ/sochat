...
function getHistory(forUser, cb) {
  messageDao.findForReceiver(forUser, (err, history) => {
    ...
    _.each(unreadMsg, (unread, client) => historyWithCountedUnreadMessage.push({
      client,
      unread
    }));
...
function getTalk(forUser, withClient, cb) {
  ...
  const receiver2 = withClient < forUser
    ? forUser
    : withClient;
  readAllMessageBetweenReceiver(receiver1, receiver2, forUser, (outerErr) => messageDao.findForSpecifiedReceiver(receiver1, receiver2, (err, messages) => {
    if (outerErr || err) {
...
function readAllMessageBetweenReceiver(receiver1, receiver2, forUser, cb) {
  messageDao.readMessage(receiver1, receiver2, forUser, (err) => (err ? console.log(err) : cb(null)));
}

function createMassage(host, client, from, time, msg) {
